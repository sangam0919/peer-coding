...
const userTableInit = async () => {
    try {
        ...
        await connectPool.query(`CREATE TABLE user (
...
const boardTableInit = async () => {
    try {
        ...
        await connectPool.query(`CREATE TABLE board (
...
const commentTableInit = async () => {
    try {
        ...
        await connectPool.query(`CREATE TABLE comment (
            uid VARCHAR(10) NOT NULL,
...
const likeaaTableInit = async () => {
    try {
        ...
        await connectPool.query(`CREATE TABLE likeaa (
    uid VARCHAR(10) NOT NULL,
    board_id INT NOT NULL,
    FOREIGN KEY (uid) REFERENCES user(uid),
    FOREIGN KEY (board_id) REFERENCES board(board_id)
